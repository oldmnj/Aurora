// Copyright (c) 2017-2026, University of Cincinnati, developed by Henry
...
CLI11_INLINE void set_unicode_locale() {
    static const std::array<const char *, 2> unicode_locales{
            {"C.UTF-8", ".UTF-8"}
    };
...
CLI11_INLINE std::string narrow_impl(const wchar_t *str, std::size_t str_size) {
    ...
    // Copy into a local, NUL-terminated buffer so the conversion is bounded by
    // str_size: a view into the middle of a larger buffer or a
    // non-NUL-terminated buffer must not be read past str_size. Note that an
    // embedded NUL inside the first str_size characters will still terminate
    // std::wcsrtombs early; this matches the behavior of C-style narrow
    // conversion and is treated as a best-effort limitation.
...
CLI11_INLINE std::wstring widen_impl(const char *str, std::size_t str_size) {
    ...
    // non-NUL-terminated buffer must not be read past str_size. Note that an
    // embedded NUL inside the first str_size characters will still terminate
    // std::mbsrtowcs early; this matches the behavior of C-style wide
    // conversion and is treated as a best-effort limitation.
